...
export function Input({invalid=false,disabled=false,style,...rest}){
  ...
  return <input disabled={disabled} onFocus={()=>setFocus(true)} onBlur={()=>setFocus(false)}
    style={{...shell,
      ...(invalid?{borderColor:"var(--feedback-critical)"}:null),
      ...(focus?{borderColor:"var(--moss-500)",boxShadow:"var(--shadow-focus)"}:null),
      ...(disabled?{background:"var(--surface-muted)",color:"var(--text-subtle)",cursor:"not-allowed"}:null),...style}} {...rest}/>;
}
